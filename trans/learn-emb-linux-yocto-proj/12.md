...
提到这些组件之后，我现在将继续解释这些工具中的每一个。 让我们从元虚拟化层的内容开始，更确切地说是`CRIU`包，这是一个在 Linux 的 Userspace中实现**检查点/恢复的项目。 它可用于冻结已在运行的应用，并将其作为文件集合检查点到硬盘驱动器。 这些检查点可用于从该点恢复和执行应用。 它可以作为许多用例的一部分使用，如下所示：**
