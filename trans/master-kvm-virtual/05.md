...
虽然虚拟机上的来宾操作系统可以通过执行主机的 CD/DVD 驱动器到虚拟机的传递从物理介质安装，但这不是最有效的方法。 与从硬盘读取 ISO 相比，从 DVD 驱动器读取较慢，因此更好的方法是将用于安装虚拟机操作系统和应用的 ISO 文件(或逻辑 CD)存储在基于文件的存储池中，并创建 ISO 映像库。
