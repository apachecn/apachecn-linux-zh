...
![Installing the audit daemon](image/B15455_11_01.jpg)
...
![Verifying of an installation of auditd and collection of logs](image/B15455_11_02.jpg)
...
![A list of commands to navigate to the VM section within the Azure portal](image/B15455_11_03.jpg)
 ...
 **![Using the ss -tulpn command to check the ports details](image/B15455_11_04.jpg)
...
![iptables-save command to include the rules created by Azure Service Fabric](image/B15455_11_05.jpg)
...
许多进程、服务和应用程序运行在典型的 Linux 系统上，这些系统会产生不同的日志，例如应用程序、事件、服务和系统日志，这些日志可用于审核和故障排除。 在前面的章节中，我们遇到了**journalctl**命令，它用于查询和显示日志。 在本章中，我们将更详细地讨论这个命令，并看看如何使用**journalctl**实用程序对日志进行切片和切片。
...
![Viewing the entries in the journal database using journalctl --unit <unit> command](image/B15455_11_06.jpg)
...
![Using the journalctl command to get the log entries of the processes](image/B15455_11_07.jpg)
...
![Filtering the log entries by using multiple parameters with journalctl](image/B15455_11_08.jpg)
...
![Viewing the recorded boots using --list-boots command](image/B15455_11_09.jpg)
...
![Get started with Log Analytics section in Azure Portal](image/B15455_11_10.jpg)

###### 图 11.10:开始使用 Azure Portal 中的 Log Analytics 部分

点击**Azure 虚拟机(vm)**。 你会在这个工作空间中看到可用的虚拟机:

![Available VMs in the workspace](image/B15455_11_11.jpg)
...
![Navigating to the View solutions option in VM](image/B15455_11_12.jpg)

###### 图 11.12:导航到监视解决方案选项

选择想要的选项，点击**添加**:

![Management Solutions within Log Analytics](image/B15455_11_13.jpg)
 ...
 **![The Summary section in Service Map](image/B15455_11_14.jpg)

###### 图 11.14:Service Map 的 Summary 部分

你可以监控你的应用程序，查看日志文件，等等:

![Check log files to moniter the application](image/B15455_11_15.jpg)
...
![Monitoring AKS cluster from the Azure portal](image/B15455_11_16.jpg)
...
![Checking Traffic Analytics tab from the Azure portal to view the network traffic flow distribution](image/B15455_11_17.jpg)
...
![Using Overview pane the Azure portal to view the performance data for the VM](image/B15455_11_18.jpg)
...
![Displaying Linux processes with top command](image/B15455_11_19.jpg)
 ...
 **![The bottom entries of the output obtained from the top command](image/B15455_11_20.jpg)
...
![Using nmon command to get Performance details of memory, CPU and storage](image/B15455_11_21.jpg)
...
![Using the glances utility to view the performance](image/B15455_11_22.jpg)
...
![Using pidstat command to get the performance on CPU data from a specific process](image/B15455_11_23.jpg)

###### 图 11.23:使用 pidstat 显示 CPU 统计信息

**iostat**是一个实用程序，顾名思义，它可以测量 I/O，但也可以创建 CPU 使用的报告:

![Using iostat command to get the performance statistics for I/O](image/B15455_11_24.jpg)
...
![Using sadf command to query the database for system activity](image/B15455_11_25.jpg)
...
![Getting real-time reports with dstat command](image/B15455_11_26.jpg)
...
![Getting the statistics for the network interface using ip -s link show dev eth0 command](image/B15455_11_27.jpg)
...
![Getting a detailed report about the ports in a listening state using nstat utility](image/B15455_11_28.jpg)
...
![Getting the network statistics with lnstat––d command](image/B15455_11_29.jpg)
...
![Menu window of IPTraf-NG](image/B15455_11_30.jpg)
...
我们相信，您现在能够在 Linux 系统中使用不同的工具来监视 CPU、内存、存储和网络细节等指标，从而实现良好的性能分析。
