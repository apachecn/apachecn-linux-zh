...
![A flowchart representing three types of owners (user, group, and others) and their associated permissions on an object, namely read, write, and execute](image/B15455_06_01.jpg)
...
![An output listing the contents of a directory](image/B15455_06_02.jpg)
...
![A glance at the data line of the directory output](image/B15455_06_03.jpg)
...
![List of access permissions and their corresponding symbols](image/B15455_06_04.jpg)

###### 图 6.4:访问权限符号

您可能想知道这个文件的所有者是谁，哪个组正在获得访问权限。 这在输出本身中得到了回答:

![Output containing information regarding owners and groups](image/B15455_06_05.jpg)
...
![Understanding the difference between various permissions for different owners](image/B15455_06_06.jpg)
...
![A list of operators for setting, adding, and removing permissions](image/B15455_06_07.jpg)

###### 图 6.7 符号方法中的运算符

这里是用户表示的列表:

![List of user denotations](image/B15455_06_08.jpg)
...
![Viewing the permissions for external.png file](image/B15455_06_09.jpg)
...
![Adding execute permissions to a user](image/B15455_06_10.jpg)
...
![Adding write and execute permissions to a group](image/B15455_06_11.jpg)
...
![An output showing change in read permissions](image/B15455_06_12.jpg)
...
![Setting read, write, and execute permissions to all owners (user, group, and others)](image/B15455_06_13.jpg)
...
![List of numeric values and their corresponding permissions](image/B15455_06_14.jpg)

###### 图 6.14:数值及其对应的权限

让我们举个例子。 检查位于当前目录中的**新文件**文件的权限。 执行**ls -lah**:

![Executing ls -lah to check permissions of new-file](image/B15455_06_15.jpg)
...
![Assigning permissions using 3-digit octal code](image/B15455_06_16.jpg)
...
![Pictorial representation of the 3-digit octal code](image/B15455_06_17.jpg)
...
![Running the sestatus command to view the state of SELinux](image/B15455_06_18.jpg)
...
![Context label for sshd process](image/B15455_06_19.jpg)
...
![Checking the status of AppArmor using the apparmor_status command](image/B15455_06_20.jpg)
...
![Configuring the profile for nginx](image/B15455_06_21.jpg)

###### 图 6.21:配置 nginx 的配置文件

**LXC**(**Linux Containers**)是一种容器技术，我们只是在为 web 服务器配置概要文件。 修正 DAC 似乎是一个不错的选择:

![Configuring the profile for a web server with DAC](image/B15455_06_22.jpg)
...
![Output of man firewalld.richlanguages command](image/B15455_06_23.jpg)
...
![Unable to save file in home directory as permission has been changed to read-only](image/B15455_06_24.jpg)
...
![Using the systemd-run and systemctl commands to collect and analyze network data](image/B15455_06_25.jpg)
...
![Changes saved in the system.control directory](image/B15455_06_26.jpg)
...
![Discovering the domain in Ubuntu- and RHEL/CentOS-based distributions](image/B15455_06_27.jpg)
...
![Using realm utility to join the domain](image/B15455_06_28.jpg)
...
![Viewing the YaST interface by running the run sudo yast command](image/B15455_06_29.jpg)

###### 图 6.29:Shell 上的 YaST 接口

最好的部分是，如果缺少任何依赖项，YaST 会提示您安装它们，所以请继续并完成依赖项的安装。 安装完成后，你可以输入你的域名，保存后，你会被提示输入用户名和密码，如下截图所示:

![Providing credentials in YaST to register the machine to Azure AD Domain Services](image/B15455_06_30.jpg)

###### 图 6.30:提供凭据来注册机器

以**user@domain**的格式输入凭证，然后输入密码。 一旦你完成了这个过程，SUSE 机器将访问 Azure AD 域服务并注册你的机器。 如果加入成功，你会在屏幕上看到如下信息:

![A message prompt indicating successful domain join](image/B15455_06_31.jpg)

###### 图 6.31:域加入成功

您可以通过使用**su**命令将当前用户切换到您的 AD 用户名来验证，如下图所示:

![Using su command to verify the domain join](image/B15455_06_32.jpg)
