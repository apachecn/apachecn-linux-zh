...
Azure AKS 使部署和管理容器应用程序变得很容易。 除了使用 Azure AKS 自动容器您的应用程序之外，您还可以快速定义、部署和调试 Kubernetes 应用程序。 您可以自动化监控、升级、修复和扩展，这减少了手动的基础设施维护。 安装了 kubectl 之后，是时候在 Azure 中设置和探索 Kubernetes 环境了:
