...
![Code to add users to the inventory file line items](image/B15455_08_01.jpg)
...
![Output of netstat and grep ssh command](image/B15455_08_02.jpg)
...
![Parametres of the created ansible file](image/B15455_08_03.jpg)

###### 图 8.3:运行 Ansible 剧本

为了确保创建了用户，我们将在所有主机中检查**/etc/passwd**文件。 从输出中，我们可以看到用户已经创建:

![Checking the password file of the hosts to verify user creation](image/B15455_08_04.jpg)
...
![User credentials indicating successful Azure login](image/B15455_08_05.jpg)
...
![Creating all the required resources for the VM with Ansible](image/B15455_08_06.jpg)
...
![Host connection failure due to authentication issues](image/B15455_08_07.jpg)
...
![Using the user credentials to ping the ansible-node3 VM](image/B15455_08_08.jpg)
...
![Screenshot indicating that ping to the user succeeds](image/B15455_08_09.jpg)
...
![Tagging the VM in the Azure portal](image/B15455_08_10.jpg)
...
![Terraform downloading the azurerm provider from its repository](image/B15455_08_11.jpg)
...
![Using terraform apply to connect terraform to the Azure subscription](image/B15455_08_12.jpg)

###### 图 8.12:连接 Terraform 到 Azure 订阅

一旦你给出了输入，Terraform 将开始创建资源。 在创建之后，Terraform 会向你展示所有创建的东西的概要，以及添加和销毁了多少资源，如下图所示:

![A summary of the resources created using the terraform apply command](image/B15455_08_13.jpg)
...
![Successful creation of the execution plan](image/B15455_08_14.jpg)
...
![Details of the resources in deployment](image/B15455_08_15.jpg)

###### 图 8.15:资源部署细节

最后，Terraform 会给出一个资源部署数量的总结:

![Summary of the deployed resources](image/B15455_08_16.jpg)

###### 图 8.16:部署的资源数量的概要

还有另一个命令，即**show**命令。 这将显示部署的完整状态，如下图所示:

![Checking the complete state of the deployment](image/B15455_08_17.jpg)
...
![Destroying the entire deployment with terraform destroy command](image/B15455_08_18.jpg)

###### 图 8.18:使用毁灭地球命令删除所有资源

最后，你会得到一个总结，如下所示:

![Summary of the destroyed resources](image/B15455_08_19.jpg)
...
![Selecting the newly created automation account](image/B15455_08_20.jpg)
...
![Product information with list of configuration directories used](image/B15455_08_21.jpg)
...
![Deploying the Guest Configuration extension onto the VM](image/B15455_08_22.jpg)
...
![OS versions for Azure Policy Guest Configuration](image/B15455_08_23.jpg)
