...
运行应用,我们首先需要确保正确版本的 Python 扩展使用 Python(我们希望 Python 3)。要做到这一点,看在状态栏底部的 Visual Studio Code窗口,直到你看到说**Python 2.7.18 64 位**或类似。 单击此部分会弹出 Python 版本选择器:
...
这个扩展使它快速和容易得到一个 Windows 终端实例打开的上下文您的Visual Studio Code项目，以保持您在流程和生产力。
