...
运行应用,我们首先需要确保正确版本的 Python 扩展使用 Python(我们希望 Python 3)。要做到这一点,看在状态栏底部的 Visual Studio 代码窗口,直到你看到说**Python 2.7.18 64 位**或类似。 单击此部分会弹出 Python 版本选择器:
...
这个截图显示了使用**Git Graph**扩展的 Git 历史。 通过使用 Visual Studio Code 扩展来呈现 Git历史，该扩展可以由在 WSL 中运行的服务器组件运行。 这允许直接访问文件以查询 Git 历史，并避免 Windows 应用的性能开销。
