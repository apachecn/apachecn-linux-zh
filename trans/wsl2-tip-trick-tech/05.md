...
这些命令显示了使用`wslview`的几个示例。 前两个示例显示根据文件扩展名启动默认 Windows 应用程序。 第一个示例启动默认的 Windows文本编辑器(通常是记事本)，第二个示例启动与 JPEG 文件关联的 Windows 应用程序。 在第三个例子中，我们传递了一个 URL，这将在默认的 Windows 浏览器中打开该 URL。
