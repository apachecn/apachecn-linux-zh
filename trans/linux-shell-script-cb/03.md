...
我们只需要从前面的细节中提取`ELF 32-bit LSB executable`。因此，我们使用`-d,`选项指定`,`作为分隔符，`-f1`选择第一个字段。
 ...
 相当于一个文件名，但是它用一个子过程输出 `<`用于输入重定向，第二个`<`用于将子流程输出转换为文件名。此外，这两者之间有一个空间，这样Shell就不会将其解释为`<<`运算符。
...
在文件系统中的多个位置导航时，一个常见的做法是将 cd 复制到您复制和粘贴的路径。如果我们处理几个地点，这是没有效率的。当我们需要在位置之间来回导航时，用每个`cd`命令键入或粘贴路径是很耗时的。Bash 和其他Shell支持`pushd`和`popd`在目录之间循环。
