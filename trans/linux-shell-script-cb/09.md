...
for 循环遍历每个 IP 地址。`ssh`应用程序向每个节点发送`df -H`命令，以检索磁盘使用信息。`df`输出存储在临时文件中。一个`while`循环逐行读取该文件，并调用`awk`提取相关数据并输出。一个`egrep`命令提取满百分比值并去除`%`。如果该值小于 80，则该行标记为`SAFE`，否则标记为`ALERT`。整个输出字符串必须重定向到`log`文件。因此，`for`循环被封装在子外壳`()`中，标准输出被重定向到日志文件。
