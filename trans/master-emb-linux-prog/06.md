...
*   `images/`：这是最重要的，因为它包含构建的结果。 根据您在配置时选择的内容，您将找到一个引导加载程序、一个内核以及一个或多个根文件系统映像。
*   `staging/`：这是指向工具链的`sysroot`的符号链接。 链接的名称有点令人困惑，因为它没有指向临时区域，正如我在[*第 5 章*](05.html#_idTextAnchor122)，*构建根文件系统*中定义的那样。
*   `target/`：这是`root`目录的临时区域。 请注意，目前您不能将其用作根文件系统，因为文件所有权和权限设置不正确。 Buildroot 使用设备表(如上一章所述)在`image/`目录中创建文件系统映像时设置所有权和权限。
...
构建完成后，映像将写入名为`output/images/sdcard.img`的文件。 用于写入镜像文件的`post-image.sh`脚本和`genimage-raspberrypi4-64.cfg`配置文件都位于`board/raspberrypi/`目录中。 要将`sdcard.img`写入 microSD 卡并在 Raspberry PI 4 上引导它，请执行以下步骤：
...
![Figure 6.1 – Selecting custom U-Boot patches](Images/B11566_06_01.jpg)
...
![Figure 6.2 – Selecting the device tree source](Images/B11566_06_02.jpg)

图 6.2-选择设备树源

我们还必须更改用于内核头的内核系列，以便它们与正在构建的内核相匹配：

![Figure 6.3 – Selecting custom kernel headers](Images/B11566_06_03.jpg)
...
![Figure 6.4 – Selecting custom scripts to run after creating filesystem images](Images/B11566_06_04.jpg)

图 6.4-选择创建文件系统映像后要运行的自定义脚本

最后，您只需输入`make`就可以为 Nova 板构建 Linux。 当它完成时，您将在`output/images/`目录中看到这些文件(以及一些附加的 DBS)：
...
`output/images/sdcard.img`写入 SD 卡，就像我们对 Raspberry PI 4 所做的那样。没有必要像我们在上一章中所做的那样预先格式化 microSD，因为`genimage`已经创建了所需的精确磁盘布局。
...
*   `core-image-x11`：这是一个基本图像，通过 X11 服务器和`xterminal`终端应用程序支持图形。
...
*   `deploy/`: This contains the final binaries to be deployed on the target:
    ...
    `deploy/images/[machine name]/`：包含准备在目标系统上运行的引导加载程序、内核和根文件系统映像。
...
![Figure 6.5 – QEMU graphic console](Images/B11566_06_05.jpg)
...
如果您查看`tmp/deploy/images/beaglebone-yocto/core-image-minimal-beaglebone-yocto.tar.bz2`，您将看到`/usr/bin/helloworld`确实已经安装。
...
您会发现`core-image-minimal`的配方在`meta/recipes-core/images/core-image-minimal.bb`中。
