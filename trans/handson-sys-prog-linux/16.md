...
最后，本章深入探讨了如何安全地混合多线程和信令，多进程和多线程的一些优缺点，以及一些提示和常见问题。
...
![](assets/624d94f5-2e34-4d0a-bfea-a95243a043eb.png)
...
![](assets/39252ce8-4494-4e42-a3b2-73a52e05d354.png)
...
在[第 11 章](11.html)、*信令-第 I 部分*和第[章](12.html)、*信令-第 II 部分*中，我们详细介绍了信令。 我们仍然在同一个 Unix/Linux 平台上；信令及其对应用程序设计/开发人员的使用不会仅仅因为我们现在正在开发 MT 应用程序而消失！ 我们仍然需要处理信号(回想一下，您可以在 shell 上使用简单的“`kill -l`”按钮列出平台的可用信号)。
...
请注意，虽然可以通过`sigaction(2)`的系统调用来捕获信号，但 MT 应用程序中信号处理的语义通常会导致使用信令 API 的阻塞变体--`sigwait(3)`、`sigwaitinfo(3)`和`sigtimedwait(3)`库 API。在我们专用的信号处理程序线程中使用这些阻塞 API 中的一个来阻塞所有需要的信号通常是一个好主意。
...
此外，要屏蔽 MT 应用程序中的信号，请不要使用`sigprocmask(2)`API-它不是线程安全的。 取而代之的是使用`pthread_sigmask(3)`库例程，也就是。
...
本章的其余部分集中在如何安全地将线程与信令接口混合；我们还比较和对比了这些方法的优缺点(真的是一些值得深思的东西)-典型的多进程单线程和多线程(只有一个进程)方法。 提示和常见问题解答完善了本章的三部曲([第 14 章](14.html)，以及本章中的*使用 PthreadsPart I-Essentials*的多线程)。
