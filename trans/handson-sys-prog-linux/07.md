...
![](assets/22f3bf7d-102d-4c63-a232-e52f883ff328.png)
...
为了便于阅读，让我们冒昧地将 GID 值从 1000 更改为(比方说)2000。 现在，如果值是 UID=1000 和 GID=2000，并且用户现在运行 vi 编辑器，那么现在的情况是这样的，请参考给定表：进程凭据-正常情况：
...
![](assets/c5b6b6b0-6a8f-4d00-a131-8fc186010ed5.png)
...
现在，我们使用以下关于 setuid passwd 可执行文件的新信息重新绘制前面的表(进程凭据-正常情况)：
...
*   当然，一旦进程结束，您就会使用常规(默认)进程凭据或权限集返回到 shell。
...
![](assets/9f6be597-f178-489f-9896-f946f56b8a9a.png)
...
我们从前面的讨论中了解到，每个活动的进程都有一组四个整数值，它们有效地确定了它的特权、真实有效的用户和组 ID；它们被称为进程凭据。

如前所述，我们将它们称为{RUID，EUID，RGID，EGID}。

有效 ID 以粗体显示，以重申这样一个事实：当实际 ID 标识原始所有者和组时，当涉及到实际检查权限时，内核使用有效 ID。

进程凭据存储在哪里？ 操作系统将此信息作为相当大的进程属性数据结构(当然是针对每个进程)的一部分保存；它位于内核内存空间中。
...
我们编写一个简单的 C 程序(`ch7/query_creds.c`)；运行时，它将打印到标准输出其进程凭据(我们显示了相关代码)：
...
但是`sudo(8)`是如何做到这一点的呢？ 我们从上一节中了解到，当您运行 ssudo(使用任何参数)时，它(至少在开始时)总是以 root 用户身份运行。 现在的问题是，它如何使用另一个用户帐户的凭据运行？
...
当然，(根)特权进程可以将其四个凭据设置为它选择的任何值。 这并不令人惊讶--这是根的力量的一部分。
...
![](assets/d3dee453-cdca-432e-a761-cb3fe1bfbcc6.png)
...
![](assets/a0736e00-379a-4550-8e77-0709bca6be26.png)
...
从前面的讨论来看，似乎`set*id()`个系统调用(`setuid(2)`、`seteuid(2)`、`setreuid(2)`、`setresuid(2)`)只对 root 有用，因为只有拥有 root 权限才能使用系统调用更改进程凭据。 嗯，这并不完全正确；对于非特权进程，还有另一个重要的案例。
...
![](assets/fda744b4-4daa-439c-b624-92f428e976c7.png)
...
非特权(即非根)进程只能使用此系统调用将这三个 ID 设置为当前 RUID、当前 EUID 或当前保存集 UID 中的一个，而不能设置其他值(通常的安全原则在起作用)。 传递`-1`意味着保持相应的值不变。 当然，特权(根)进程可以使用该调用将这三个 ID 设置为任何值。 (通常，`setresgid(2)`系统调用是相同的，只是它设置了组凭据)。
